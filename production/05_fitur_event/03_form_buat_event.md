# 03 — Form Buat Event Baru

> **Prasyarat:** Dashboard admin sudah diimplementasi (file `02_dashboard_admin.md` sudah selesai).
> **File target:** `src/pages/admin/CreateEventPage.tsx`

---

## Tujuan

Mengimplementasikan form multi-section untuk membuat event baru: Informasi Event + Pengaturan Email + Upload Template.

---

## Implementasi `src/pages/admin/CreateEventPage.tsx`
...
- Template sertifikat PDF belum diupload di tahap ini — akan dilakukan di halaman **Template Editor** (folder `06_template_editor`).
- Event dibuat dengan status `draft` secara default.
- Prefix dikonversi otomatis ke UPPERCASE.

---

## Kriteria Selesai

- [ ] Form buat event berfungsi dan menyimpan ke Supabase
- [ ] Validasi inline berfungsi (field wajib, format prefix)
- [ ] Setelah submit berhasil, redirect ke halaman detail event
- [ ] Loading state tampil saat submit
- [ ] Error message tampil jika gagal
- [ ] Record `event_email_settings` dan `certificate_templates` otomatis dibuat
- [ ] Record `event_status_history` dicatat (status: draft)
