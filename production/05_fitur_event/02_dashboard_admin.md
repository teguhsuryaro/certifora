# 02 — Dashboard Admin & Daftar Event

> **Prasyarat:** Service dan store event sudah dibuat (file `01_service_dan_store.md` sudah selesai).
> **File target:** `src/pages/admin/DashboardPage.tsx`

---

## Tujuan

Mengimplementasikan halaman dashboard admin yang menampilkan daftar semua event dalam bentuk grid card, dengan filter status dan tombol buat event baru.

---

## Implementasi `src/pages/admin/DashboardPage.tsx`
...
- [ ] Dashboard menampilkan grid card event
- [ ] Filter status berfungsi (Semua/Draft/Aktif/dll)
- [ ] Empty state tampil jika belum ada event
- [ ] Tombol "Buat Event Baru" berfungsi (link ke `/admin/events/new`)
- [ ] Card event menampilkan nama, status badge, tanggal, penyelenggara, jumlah peserta
- [ ] Klik card mengarah ke detail event
- [ ] FAB muncul di mobile
- [ ] Layout responsive (1 kolom mobile, 2 kolom tablet, 3 kolom desktop)
