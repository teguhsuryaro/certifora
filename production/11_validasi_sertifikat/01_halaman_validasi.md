# 01 — Halaman Validasi Sertifikat

> **Prasyarat:** Pengiriman email sudah selesai (folder `10_kirim_email` selesai).
> **File target:** `src/pages/public/VerifyPage.tsx`

---

## Tujuan

Mengimplementasikan halaman validasi sertifikat publik yang diakses via QR code pada sertifikat. Mobile-first, sederhana, fokus pada informasi validasi.

---

## Implementasi `src/pages/public/VerifyPage.tsx`
...
- **Mobile-first:** Diakses via scan QR dari sertifikat (kebanyakan lewat HP)
- **Centered card:** Fokus tunggal pada informasi validasi
- **Kode sertifikat** ditampilkan dengan `font-mono` dan `tracking-wide` agar mudah dibaca/dicocokkan
- **Selalu aktif:** Halaman validasi tetap berfungsi meskipun event sudah ditutup permanen (sesuai spesifikasi)
- **Tidak ada navigasi:** Halaman berdiri sendiri, dibuka sekali → dibaca → selesai

---

## Kriteria Selesai

- [ ] Halaman validasi menampilkan status valid/tidak valid
- [ ] Jika valid: menampilkan nama, event, penyelenggara, tanggal, kode
- [ ] Jika tidak valid: menampilkan pesan error dengan kode yang dimasukkan
- [ ] Query gabungan `certificate_codes` → `participants` → `events` berfungsi
- [ ] Halaman mobile-first dengan centered card
- [ ] Kode sertifikat tampil dalam font monospace
- [ ] Footer branding "Diverifikasi oleh Certifora" tampil
- [ ] Loading state tampil saat query berlangsung
- [ ] Validasi tetap berfungsi meskipun event ditutup permanen
