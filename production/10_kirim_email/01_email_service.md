# 01 — Logika Pengiriman Email Sertifikat

> **Prasyarat:** PDF generator sudah selesai (folder `09_generate_pdf` selesai).
> **File target:** `src/services/emailService.ts`

---

## Tujuan

Mengimplementasikan service untuk mengirim email sertifikat melalui Supabase Edge Function, termasuk:
- Kirim per peserta
- Kirim semua (batch) dengan batasan harian
- Progress tracking
...
Tambahkan logika "Kirim Semua" di `ParticipantsPage.tsx` dengan modal progress:

**Komponen tambahan yang perlu dibuat:** Modal progress yang menampilkan:
- Progress bar real-time
- Nama peserta yang sedang diproses
- Counter: "Mengirim 23 dari 87..."
- Pesan jika limit harian tercapai
- Tombol close setelah selesai

**Flow:**
1. Klik "Kirim Semua"
2. Modal konfirmasi: "Akan mengirim sertifikat ke X peserta yang belum terkirim"
3. Setelah konfirmasi, modal progress tampil
4. Proses berjalan satu per satu (generate PDF → kirim email)
5. Progress bar update real-time
6. Jika limit harian tercapai: "Hari ini terkirim X dari Y. Sisanya tersedia besok."
7. Setelah selesai: refresh data peserta

---

## Kriteria Selesai

- [ ] Service kirim email per peserta berfungsi (generate PDF → Edge Function → Resend)
- [ ] Service kirim semua (batch) dengan progress callback berfungsi
- [ ] Batching harian: max 90 email/hari, cek kuota sebelum mulai
- [ ] Delay 500ms antar pengiriman (hindari rate limit)
- [ ] Delivery logs terupdate (success/failed)
- [ ] Participant delivery_status terupdate
- [ ] Pesan jelas jika limit harian tercapai
- [ ] Template PDF di-fetch sekali untuk batch (bukan per peserta)
